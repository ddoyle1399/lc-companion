...
async function getPoetryRows(): Promise<NoteRowData[]> {
  ...
  return data.map((r: any) => ({
    ...
    href: `/library/poetry/${r.id}`,
  }));
}

export default async function PoetryLibraryPage() {
  const [rows, counts] = await Promise.all([getPoetryRows(), getLibraryCounts()]);
  return (
    <div className="min-h-screen bg-cream">
      ...
      <main className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {rows.length === 0 ? (
          <EmptyState message="No poetry notes generated yet." cta={{ href: "/poetry", label: "Generate poetry" }} />
        ) : (

 ...
 * poetry prompt depends on. This is a structural check, not a content check.
 *
...
export function validatePoetryMetadata(ctx: PromptContext): MetadataValidationResult {
  ...
  // Schema version gate. Anything pre-v2 is not ready for the hardened prompt.
  ...
  // Rich blocks the prompt and critic both depend on.
...
export function buildPoetryNotePrompt(ctx: PromptContext): {
  ...
  if (!validation.ok) {
    throw new MetadataIncompleteError(
      ctx.subject ?? '',
      ctx.subKey ?? '',
      validation.missing
    );
